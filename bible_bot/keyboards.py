...
def completion_keyboard(has_favorites: bool) -> InlineKeyboardMarkup:
    rows = [
        [InlineKeyboardButton(text="🔄 Начать личный круг", callback_data="cycle:restart")],
        [InlineKeyboardButton(text="📖 Читать с Матфея", callback_data="cycle:sequential")],
        [InlineKeyboardButton(text="🎯 Выбрать тему", callback_data="cycle:themes")],
    ]
    if has_favorites:
        rows.append(
            [InlineKeyboardButton(text="🤍 Мои сохранённые главы", callback_data="favorites:show")]
        )
