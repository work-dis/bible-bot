...
Telegram-бот присылает каждый день одну полную главу Нового Завета в
Синодальном переводе. Пользователь выбирает время и часовой пояс; длинные главы
автоматически делятся на несколько последовательных сообщений из-за лимита
Telegram.
...
3. Первая глава приходит сразу, плановая рассылка начинается на следующий день.
4. В карточке видны книга, глава и прогресс цикла; под последней частью доступны
   выбор стихов, отправка размышлений, сохранение главы и настройки.
5. Размышления можно отправить текстом, аудио или видео. Бот публикует их в
   открытом Telegram-канале с именем автора, главой и выбранными стихами.
6. После Откровения 22 бот показывает число сохранённых глав и ставит рассылку
   на паузу.

Основной цикл содержит все 260 глав Нового Завета в каноническом порядке. В
общем режиме глава дня одна для всех, но приходит в локальное время пользователя.
Тематические циклы также выдают полные главы, связанные с верой, надеждой,
любовью, молитвой, словами Иисуса, утешением и отношениями с людьми.
...
- Основной корпус `new_testament_chapters.json` хранит 260 глав: одна строковая
  запись на главу. Отдельной базы стихов и стиховых ключей в плане нет.
...
администратором с правом публикации сообщений и укажите username канала в
`PUBLIC_CHANNEL_ID`. Если у публикаций должна быть кнопка «Комментарии»,
привяжите к каналу группу обсуждения в настройках Telegram. Все отправленные
через кнопку «Написать размышления» тексты, аудио и видео будут видны публично;
бот заранее предупреждает об этом пользователя.
...
- Ввод произвольного времени и зоны хранится в Neon и переживает холодный
  запуск Vercel.
- Выбор стихов и ожидание размышления также хранятся в базе и переживают
  холодный запуск.
...
bible_bot/
  ...
  content.py                 чтение корпуса и выбор ежедневных глав
  data/                      корпус по главам и план чтения
scripts/
