...
бот заранее предупреждает об этом пользователя. После публикации бот предлагает
...
- `/channel` — ссылка на публичный канал;
...
- Выбор стихов и ожидание размышления также хранятся в базе и переживают
  холодный запуск.
- Одноразовые служебные рассылки записывают результат по каждому пользователю,
  поэтому повторный запуск не дублирует уже доставленные сообщения.
...
## Одноразовое уведомление об обновлении

Сначала безопасно проверьте число получателей и текст без отправки:

```bash
export BOT_TOKEN=...
export DATABASE_URL=postgresql://...
python scripts/broadcast_update.py --dry-run
```

Затем запустите рассылку:

```bash
python scripts/broadcast_update.py
```

Скрипт соблюдает Telegram rate limit, пропускает пользователей, заблокировавших
бота, и не отправляет кампанию повторно тем, кто уже получил уведомление.
